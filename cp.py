...
def mainWindow():
    ...
    root = Tk()
    root.config(bg="#16213E")
    root.title("CRYPTOASS")
    root.geometry("700x450+0+0")
    root.resizable(False, False)
    Tops = Frame(root, width=700, height=50, bd=16, relief="flat", bg="#16213E")
    Tops.pack(side=TOP)
    mainTitle = Label(Tops, text='Password Generator', borderwidth=1, relief="flat",
                      fg="white", bg="#16213E", font=('arial', 15, 'bold'), bd=10, anchor='w')
    mainTitle.grid(row=0, column=0)
    LF = Frame(root, width=400, height=450, bd=16, relief="sunken", bg="#16213E")
    LF.pack(side=LEFT)
    RF = Frame(root, width=300, height=450, bd=16, relief="flat", bg="#16213E")
    RF.pack(side=RIGHT)
    ...
    select_uppercase = Checkbutton(LF, text="Include Uppercase Alphabets", variable=var_uppercase,
                                   onvalue=1, offvalue=0,bg="#16213E", fg="#fff").grid(row=1, column=0, padx=10, pady=10)
    select_special_char = Checkbutton(LF, text="Include Special Characters", variable=var_special,
                                      onvalue=1, offvalue=0,bg="#16213E", fg="#fff").grid(row=2, column=0, padx=10, pady=10)
    select_numerical_char = Checkbutton(LF, text="Include Numerical Characters",
                                        variable=var_numerical, onvalue=1, offvalue=0, bg="#16213E", fg="#fff").grid(row=3, column=0, padx=10, pady=10)
    resultant_password_txt = Label(
        LF, text='Resultant Password:', fg="#fff", bg="#16213E", font=('arial', 11, 'bold'))
    resultant_password_txt.grid(row=4, column=0, padx=10, pady=10)
    resultant_password = Label(
        LF, text='gsgdhg2hh3h&@345', fg="white", bg="#16213E", font=('arial', 11, 'bold'), width=16)
    ...
    generate_btn = Button(RF, text="GENERATE RANDOM PASSWORD", command=generatePassword,
                          pady=6, bd=8, fg="#fff", bg="#064663",font=('arial', 10, 'bold'), width=26)
    ...
    reset_btn = Button(RF, text="RESET", command=resetPreferences, pady=6,
                       bd=8, fg="#fff", bg="#064663",font=('arial', 10, 'bold'), width=26)
...
def passwordManager(rt):
    ...
    Tops = Frame(pwd_manager, width=700, bg="#16213E",height=50, bd=16, relief="flat")
    Tops.pack(side=TOP)
    mainTitle = Label(Tops, text='Password Manager', borderwidth=1, relief="flat",
                      fg="white", bg="#16213E", font=('arial', 15, 'bold'), bd=10, anchor='w')
    mainTitle.grid(row=0, column=0)
    LF = Frame(pwd_manager, width=400, height=450, bd=16, relief="sunken", bg="#16213E")
    LF.pack(side=LEFT)
    RF = Frame(pwd_manager, width=300, height=450, bd=16, relief="flat", bg="#16213E")
    RF.pack(side=RIGHT)
    details = Label(LF, text='Enter the details',
                    fg="white", font=('arial', 12, 'bold'), bg="#16213E")
    details.grid(row=0, column=0, padx=10, pady=10)
    email = Label(
        LF, text='Email', fg="white", font=('arial', 11, 'bold'), bg="#16213E")
    email.grid(row=1, column=0)
    txtEmail = Entry(LF, font=('arial', 11, 'bold'), width=26,
                     bg="white", justify='left', textvariable=email_text)
    txtEmail.grid(row=1, column=1)
    nil1 = Label(LF, text='',
                    fg="white", font=('arial', 12, 'bold'), bg="#16213E")
    nil1.grid(row=2, column=0)
    nil2 = Label(LF, text='',
                    fg="white", font=('arial', 12, 'bold'), bg="#16213E")
    nil2.grid(row=2, column=1)
    password = Label(
        LF, text='Password', fg="white", font=('arial', 11, 'bold'), bg="#16213E")
    password.grid(row=3, column=0)
    txtPassword = Entry(LF, font=('arial', 11, 'bold'), width=26,
                        bg="white", justify='left', textvariable=password_text, show='*')
    txtPassword.grid(row=3, column=1)
    nil3 = Label(LF, text='',
                    fg="white", font=('arial', 12, 'bold'), bg="#16213E")
    nil3.grid(row=4, column=0)
    nil4 = Label(LF, text='',
                    fg="white", font=('arial', 12, 'bold'), bg="#16213E")
    nil4.grid(row=4, column=1)
    username = Label(
        LF, text='Username', fg="white", font=('arial', 11, 'bold'), bg="#16213E")
    username.grid(row=5, column=0)
    txtUsername = Entry(LF, font=('arial', 11, 'bold'), width=26,
                        bg="white", justify='left', textvariable=username_text)
    txtUsername.grid(row=5, column=1)
    nil5 = Label(LF, text='',
                    fg="white", font=('arial', 12, 'bold'), bg="#16213E")
    nil5.grid(row=6, column=0)
    nil6 = Label(LF, text='',
                    fg="white", font=('arial', 12, 'bold'), bg="#16213E")
    nil6.grid(row=6, column=1)
    url = Label(
        LF, text='URL / App Name', fg="white", font=('arial', 11, 'bold'), bg="#16213E")
    url.grid(row=7, column=0)
    txtURL = Entry(LF, font=('arial', 11, 'bold'), width=26,
                   bg="white", justify='left', textvariable=url_text)
    txtURL.grid(row=7, column=1)
    save_btn = Button(RF, text="SAVE", command=saveData,
                      pady=6, bd=8,fg="#fff", bg="#064663", font=('arial', 10, 'bold'), width=26)
    save_btn.grid(row=0, column=0, padx=5, pady=5)
    reset_btn = Button(RF, text="RESET", command=resetDataFields,
                       pady=6, bd=8, fg="#fff", font=('arial', 10, 'bold'), width=26, bg="#064663")
...
def getPassword(rt):
    ...
    retrieve_pwd.config(bg="#16213E")
    retrieve_pwd.title("RETRIEVE PASSWORD")
    retrieve_pwd.geometry("750x450+0+0")
    retrieve_pwd.resizable(False, False)
    url_text = StringVar()
    Tops = Frame(retrieve_pwd, width=750, height=50, bd=16, relief="flat", bg="#16213E")
    Tops.pack(side=TOP)
    mainTitle = Label(Tops, text='Retrieve Password', borderwidth=1, relief="flat",
                      fg="white", bg="#16213E", font=('arial', 15, 'bold'), bd=10, anchor='w')
    mainTitle.grid(row=0, column=0)
    LF = Frame(retrieve_pwd, width=400, height=450, bd=16, bg="#16213E",relief="sunken")
    LF.pack(side=LEFT)
    RF = Frame(retrieve_pwd, width=350, height=450, bd=16,bg="#16213E", relief="flat")
    RF.pack(side=RIGHT)
    details = Label(LF, text='Enter the URL / App name',
                    fg="white",bg="#16213E", font=('arial', 11, 'bold'))
    details.grid(row=0, column=0, padx=5, pady=5)
    url = Label(
        LF, text='URL / App Name', fg="white",bg="#16213E", font=('arial', 11, 'bold'))
    url.grid(row=1, column=0)
    txtURL = Entry(LF, font=('arial', 11, 'bold'), width=26,
                   bg="white",fg='black', justify='left', textvariable=url_text)
    txtURL.grid(row=1, column=1)
    get_password = Button(RF, text="GET", command=retrievePasswordFromDB,
                          pady=6, bd=8, fg="white",bg="#064663", font=('arial', 10, 'bold'), width=26)
    ...
    your_password_txt = Label(
        LF, text='Your Password:', fg="white",bg="#16213E", font=('arial', 11, 'bold'))
    your_password_txt.grid(row=4, column=0, padx=10, pady=10)
    your_password = Label(
        LF, text='', fg="white",bg="#16213E", font=('arial', 11, 'bold'), width=16)
...
def showAllPasswords(rt):
    ...
    show_pwds.config(bg="#16213E")
    show_pwds.title("SHOW ALL PASSWORDS")
    show_pwds.geometry("1000x500+0+0")
    show_pwds.resizable(False, False)
    url_text = StringVar()
    Tops = Frame(show_pwds, width=1000, height=50,bg="#16213E",  relief="flat")
    Tops.pack(side=TOP)
    mainTitle = Label(Tops, text='ALL PASSWORDS', borderwidth=1, relief="flat",
                      fg="#fff", bg="#16213E", font=('arial', 15, 'bold'), bd=10, anchor='w')
    mainTitle.grid(row=0, column=0)
    CF = Frame(show_pwds, width=1000, height=500,  relief="sunken")
    CF.pack(side=TOP)
    email = Label(CF, text="Email", fg="white",bg="#064663",
                  borderwidth=2, relief="raised", width=14)
    email.grid(row=0, column=0, padx=10, pady=10)
    password = Label(CF, text="Password",
                  fg="white",bg="#064663", borderwidth=2, relief="raised", width=14)
    ...
    back_btn = Button(CF, text="BACK", command=lambda: reinstate(rt, show_pwds),
                      pady=6, bd=8, fg="#fff", bg="#064663",font=('arial', 10, 'bold'), width=20)
